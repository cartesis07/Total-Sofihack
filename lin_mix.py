...
headers = ['# Consommation (MW)', 'Thermique (MW)', 'Nucléaire (MW)', 'Eolien (MW)', 'Solaire (MW)', 'Hydraulique (MW)', 'Pompage (MW)', 'Bioénergies (MW)']
...
for d in date_ar:
    while (d - r_date).seconds > 0:
        current_row += 1
        r_date = datetime.strptime(R[current_row][1]['date'], '%Y-%m-%d %H:%M:%S')
        result_line = dict()
        for h in headers:
            a = R[current_row][1]['reg_a_' + p]
            b = R[current_row][1]['reg_b_' + p]
            t = d.timestamp()
            r = a*t+b
            result_line['date'] = d
